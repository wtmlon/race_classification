    ...
    for pic in pics:
        ...
        for face_location in face_locations:
            ...
            if width == 178 and height == 218:
                pil_image.save(new_path + '\\' + 'm_{:0>7d}.jpg'.format(cnt), quality = 100)
                cnt = cnt + 1
...
def lock_test(path, new_path, cnt):
    mu = threading.Lock()
    if mu.acquire(True):
        process_img(path, new_path, cnt)
        mu.release()
 
if __name__ == '__main__':
    paths = [r'F:\RaceDataSet\African']
    new_paths = [r'F:\RaceDataSet\African1']
    for i in range(len(paths)):
        my_thread = threading.Thread(target=lock_test, args=(paths[i], new_paths[i], i+1))
        my_thread.start()